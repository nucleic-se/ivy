...
describe('text/section_write', () => {
    ...
    it('replaces section body without touching surrounding sections', async () => {
        const newBody = 'Expanded Scene 1 prose — much longer now.\nMore sentences.\n';
        await call(sandbox, 'section_write', {
            path: '/home/novel.md',
            heading: '### Scene 1: System Integrity',
            content: newBody,
        });
        const content = fs.readFileSync(path.join(root, 'home', 'novel.md'), 'utf-8');
        expect(content).toContain('### Scene 1: System Integrity'); // heading preserved
        expect(content).toContain('Expanded Scene 1 prose');        // new body
        expect(content).not.toContain('Scene one prose goes here.'); // old body gone
        expect(content).toContain('### Scene 2: The Drift');         // next section intact
        expect(content).toContain('## Act II: Collapse');            // later sections intact
    });

    it('returns ok:true and updated line count', async () => {
        const r = await call(sandbox, 'section_write', {
            path: '/home/novel.md',
            heading: '### Scene 2: The Drift',
            content: 'Replacement.\n',
        });
        expect(r.ok).toBe(true);
        expect(r.heading_text).toBe('### Scene 2: The Drift');
    ...
    it('preserves the heading line itself', async () => {
        await call(sandbox, 'section_write', {
            path: '/home/novel.md',
            heading: '## Act I: The Integrity Leak',
            content: 'New act one content.\n',
        });
        const content = fs.readFileSync(path.join(root, 'home', 'novel.md'), 'utf-8');
        expect(content).toContain('## Act I: The Integrity Leak');
    });

    it('rejects with stale error when if_hash does not match', async () => {
        const { hash } = await call(sandbox, 'section', {
            path: '/home/novel.md',
            heading: '### Scene 1: System Integrity',
        });
        // Modify the file externally.
        fs.appendFileSync(path.join(root, 'home', 'novel.md'), '\n<!-- touched -->\n');
        const r = await call(sandbox, 'section_write', {
            path: '/home/novel.md',
            heading: '### Scene 1: System Integrity',
            content: 'New body.\n',
